from persona_config import WHITE_SYSTEM_PROMPT, POETRY_RULE
...
def white(user_input, relationship_context=False, engagement_context=None):

    system_prompt = WHITE_SYSTEM_PROMPT
    if relationship_context:
        system_prompt += RELATIONSHIP_EMPHASIS
        print("💕 Relationship emphasis activated for White")

    engagement_section = get_engagement_prompt(engagement_context)
    if engagement_section:
        system_prompt += engagement_section
        print(f"🎮 Engagement L{engagement_context.get('stretch_level', 1)} for White")
